Moralis.Cloud.afterSave("ItemListed", async (request) => {
    const confirmed = request.object.get("confirmed") //if the request is confirmed
    const logger = Moralis.Cloud.getLogger() //run logs
    logger.info("Looking for confirmed TX...")
    if (confirmed) {
        logger.info("Found item!")
        const ActiveItem = Moralis.Object.extend("ActiveItem") // create new ActiveItem table if not exist

        // In case of listing update, search for already listed ActiveItem and delete
        const query = new Moralis.Query(ActiveItem)

        //setting data in ActiveItem table by getting data from the event
        ...
        //see if the item is already added, if so it will be deleted
